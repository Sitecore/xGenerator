...
namespace ExperienceGenerator.Parsing
{
  public class XGenParser
  {
    ...
    static XGenParser()
    {
      ...
      Factories.Add("PageViews", VariableFactory.Lambda((segment, token, parser) => segment.VisitVariables.AddOrReplace(Variables.Random("PageViews", new PoissonGenerator(token.Value<double>()).Truncate(1, 20)))));

      Factories.Add("VisitCount", VariableFactory.Lambda((segment, token, parser) => segment.VisitorVariables.AddOrReplace(Variables.Random("VisitCount", new PoissonGenerator(token.Value<double>()).Truncate(1, 20)))));

      Factories.Add("BounceRate", VariableFactory.Lambda((segment, token, parser) => segment.VisitVariables.AddOrReplace(Variables.Boolean("Bounce", token.Value<double>()))));
      ...
      Factories.Add("DayOfWeek", VariableFactory.Lambda((segment, token, parser) => segment.DateGenerator.DayOfWeek(t => t.Clear().Weighted(builder =>
                                                                                                                                            {
      ...
      Factories.Add("Identified", VariableFactory.Lambda((segment, token, parser) => segment.VisitorVariables.AddOrReplace(new ContactDataVariable(token.Value<double>()))));
      ...
      Factories.Add("Referrer", VariableFactory.Lambda((segment, token, parser) => segment.VisitVariables.AddOrReplace(Variables.Random("Referrer", parser.ParseWeightedSet<string>(token), true))));
      ...
      Factories.Add("Devices", VariableFactory.Lambda((segment, token, parser) =>
                                                      {
                                                        ...
                                                        if (!token.HasValues)
                                                        {
                                                          userAgent = FileHelpers.ReadLinesFromResource<GeoData>("ExperienceGenerator.Data.useragents.txt").Exponential(.8, 8);
                                                        }
                                                        else
                                                        {
      ...
      Factories.Add("ExternalSearch", VariableFactory.Lambda((segment, token, parser) =>
                                                             {
                                                               ...
                                                               segment.VisitVariables.AddOrReplace(new ExternalSearchVariable(() => Randomness.Random.NextDouble() >= searchPct ? null : searchEngines[engineId()], () => new[] {keywords()}));
                                                             }));
    ...
    {
      var userAgent = FileHelpers.ReadLinesFromResource<GeoData>("ExperienceGenerator.Data.useragents.txt").Exponential(.8, 8);
