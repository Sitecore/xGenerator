...
namespace Colossus.Integration.Processing
{
    public class TriggerEventsAction : IRequestAction
    {
        ...
        {
            var events = requestInfo.Variables.GetOrDefault(VariableKey.TriggerEvents) as IEnumerable<TriggerEventData>;
            if (events == null)
                return;
            foreach (var e in events)
            {
                var eventData = tracker.Interaction.CurrentPage.Register(new PageEventData(e.Name, e.Id ?? Guid.Empty)
                                                                         {
                                                                             Text = e.Text
                                                                         });
