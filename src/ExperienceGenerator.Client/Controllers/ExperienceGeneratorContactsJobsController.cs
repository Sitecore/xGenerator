...
{
    public class ExperienceGeneratorContactsJobsController : ApiController
    {        
        public IHttpActionResult Post([FromBody] JobSpecification spec)
        {
            spec.Type = JobType.Contacts;
            ...
            return RedirectToRoute("ExperienceGeneratorContactsJobsApi", new { id = status.Id });
        }
        ...
        {
            job.StatusUrl = Url.Route("ExperienceGeneratorContactsJobsApi", new { action = "get", id = job.Id });
            return job;
        }
    }
}