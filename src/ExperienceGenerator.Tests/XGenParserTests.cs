namespace ExperienceGenerator.Tests
{
  ...
  public class XGenParserTests
  {
    ...
    [Fact]
    [AutoDbData]
    public void DevicesFactory_ShouldUpdateSegment()
    {
      var factory = XGenParser.Factories["Devices"];
      var parser = new XGenParser("http://google.com");
      var devices = new DeviceRepository().GetAll();
      ...
      {
        var s = new VisitorSegment("");
        factory.UpdateSegment(s, definition, parser);
        var so = new Request();
        s.VisitorVariables.First(x => x.ProvidedVariables.Contains("UserAgent")).SetValues(so);
        userAgentList.Add(so.Variables["UserAgent"].ToString());
      ...
      foreach (var device in definition)
      {
        ...
        (userAgentList.Count(x => x == dev.UserAgent) / 1000d).Should().BeApproximately(expected, 0.05);
